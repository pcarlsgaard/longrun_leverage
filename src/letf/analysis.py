...
from .provenance import FLOAT_FORMAT, sha, stable_floats
...
def run(root, offline=False, refresh=False):
    ...
    for name, rows in outputs.items():
        pd.concat(rows, ignore_index=True).pipe(stable_floats).to_csv(reports/f'{name}.csv', index=False, float_format=FLOAT_FORMAT)
    for name, rows in [('portfolio', static_cohorts), ('sma', sma_cohorts)]:
        pd.concat(rows, ignore_index=True).pipe(stable_floats).to_csv(processed/f'{name}_rolling_cohorts.csv', index=False, float_format=FLOAT_FORMAT)
    pd.DataFrame(daily_export).pipe(stable_floats).to_csv(processed/'portfolio_sma_daily_returns.csv', index_label='date', float_format=FLOAT_FORMAT)
    pd.DataFrame(state_export).pipe(stable_floats).to_csv(processed/'sma_daily_positions.csv', index_label='date', float_format=FLOAT_FORMAT)
    daily[[CASH]].pipe(stable_floats).to_csv(processed/'tbill_daily_returns.csv', index_label='date', float_format=FLOAT_FORMAT)
