...
def run(root, config, refresh=False, offline=False):
    ...
    daily.pipe(stable_floats).to_csv(processed / "daily_returns.csv", float_format=FLOAT_FORMAT)
    ...
    levels.pipe(stable_floats).to_csv(processed / "wealth_indices.csv", float_format=FLOAT_FORMAT)
    provenance.to_csv(processed / "daily_source_labels.csv", index_label="date")
    costs.pipe(stable_floats).to_csv(processed / "daily_cost_components.csv", index_label="date", float_format=FLOAT_FORMAT)
